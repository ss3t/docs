...
| # color-primary-shadow [{color}](#color)    | Цвет тени (при выделение) интерактивных элементов описанных выше                              |
...
Для их использоваия так же нужно проставлять префиксы вместе с глобальными префиксами
...
```
Мы получаем `https://fonts.googleapis.com/css?family=Baloo+2|Roboto&display=swap`, затем вставляем тег с ссылкой на шрифт в игру:
```
...
2. HEX. Пример `#C9F63A`
3. RGBA. Пример `rgba(0, 255, 0, 0.8)`
4. HSLA. Пример `hsla(120, 100%, 50%, 0.8)`
...
4. `%` В процентах. Так же как и `em` значение берется от родителя но в процетах. Пример `130%`
4. `pt` В типографских пунктах (4/3 px). Пример `20pt`
5. `pc` В типографских пиках (16px). Пример `1.5pc`
6. `mm` В милиметрах (3.8px). Пример `7mm`
7. `cm` В милиметрах (38px). Пример `0.5cm`
...
6. `em` В зависимости от родителя. Например если выставить значение `1.5em`, а изходные размер текста у игры стоит `10px`, то получится `15px`
...
Так же доступные все еденицы измерения описанные в [font-size](#font-size)
...
см. [раздел](#%D0%B8%D1%81%D0%BF%D0%BEn%D1%8C%D0%B7%D0%BE%D0%B2%D0%B0%D0%BD%D0%B8%D0%B5-%D1%88%D1%80%D0%B8%D1%84%D1%82%D0%BE%D0%B2-%D1%81-google-fonts) по установки и использыванию шрифтов с сервиса Google Fonts
...
В интернете можно найте генераторы градиентов, например https://cssgradient.io/
